...
int format(FILE *fp) {
    ...
    super.blockNumber = ftell(fp) / super.blockSize;
    // ftell获取文件流中指针的当前值,此处就是文件的字节长度
    ...
    while (path != NULL) {
        ...
        for (int i = 0; i < 10; i++) {
            if (root->dataPoint[i] != 0) {
                ...
                for (int j = 0; j < 16; j++) {
                    // 遍历这个块中的16个FileNode
                    if (nodePointer[j].valid == 1 && nodePointer[j].type == 1 &&
                        strcmp(nodePointer[j].filename, path) == 0) {
                        // 找到该FileNode
                        flag = 1;
                        targetNode = nodePointer + j;
                        targetOffset =
                            j * 256 + BLOCK_SIZE * root->dataPoint[i];
                        break;
                    }
                    if (nodePointer[j].valid == 1 && nodePointer[j].type == 0 &&
                        strcmp(nodePointer[j].filename, path) == 0) {
                ...
                if (((long *)blockNode)[i] != 0) {
                    ...
                    for (int j = 0; j < 16; j++) {
                        if (nodePointer[j].valid == 1 &&
                            ...
                            targetNode = nodePointer + j;
                            targetOffset =
                                j * 256 + BLOCK_SIZE * ((long *)blockNode)[i];
                            break;
                        }
                        if (nodePointer[j].valid == 1 &&
                            nodePointer[j].type == 0 &&
                            strcmp(nodePointer[j].filename, path) == 0) {
                            // 找到了一个和path同名的文件，而不是目录，这种情况下，其实是执行不下去的
                            // 有两种情况，第一就是输入的路径有问题
                            // 第二就是当前的path已经是最终的文件名了，那么此时，待创建的文件是已经存在的
        ...
        if (flag == 0) {
            ...
            if (path == NULL && fileExist == 1) {
                // 表明
                // 上面遍历的循环是由于fileExist退出的，并且此时的path是最后一个字符串了，即待创建的文件是存在的
                free(file);
                free(block);
                free(blockNode);
                return -5;
            ...
            if (fileExist == 1) {
                // 表明
                // 是中间的某一级目录不存在，并且在该目录应该存在的位置存在一个与目录重名的文件
                free(file);
                free(block);
                free(blockNode);
                return -6;
            ...
            if (path == NULL) {
                // 表明当前是最后一个path中的字符串了 即待创建的文件名
                // 该文件名为localPath，应该在root这个目录下面
                ...
                for (int i = 0; i < 10; i++) {
                    if (root->dataPoint[i] == 0) {
                        ...
                        nodePointer[0].belong = currentUid;
                        ...
                        // 这个块存在，我们遍历这个块的16个FileNode,如果有无效的(valid
                        // == 0),就写入到这个里面去
                        fseek(fp, BLOCK_SIZE * root->dataPoint[i], SEEK_SET);
                        fread(block, BLOCK_SIZE, 1, fp);
                        for (int j = 0; j < 16; j++) {
                            if (nodePointer[j].valid == 0) {
                                ...
                                memset(&nodePointer[j], 0,
                                       sizeof(struct FileNode));
                                nodePointer[j].valid = 1;
                                nodePointer[j].belong = currentUid;
                        ...
                        if (createSuccess == 1) {
                            fseek(fp, BLOCK_SIZE * root->dataPoint[i],
                                  SEEK_SET);
                            fwrite(block, BLOCK_SIZE, 1, fp);
                            break;
                        }
...
int createDir(const char *dirPath, FILE *fp) {
    ...
    while (path != NULL) {
        ...
        if (flag == 0) {
            ...
            if (path == NULL && fileExist == 1) {
                // 表明
                // 上面遍历的循环是由于fileExist退出的，并且此时的path是最后一个字符串了，即待创建的目录存在一个同名的文件，创建失败
                free(dir);
                free(block);
                free(blockNode);
                return -5;
            ...
            if (path == NULL) {
                // 表明当前是最后一个path中的字符串了 即待创建的目录名
                // 该目录名为localPath，应该在root(我们每次循环都会更新的)这个目录下面
...
long openFile(const char *filePath, FILE *fp, Ui::MainWindow *ui) {
    ...
    while (path != NULL) {
            ...
            if (root->dataPoint[i] != 0) {
                ...
                for (int j = 0; j < 16; j++) {
                    ...
                    if (nodePointer[j].valid == 1 && nodePointer[j].type == 0 &&
                        ...
                        fileExist = 1;
                        nodePointer +=
                            j; // 这样的话，我们的nodePointer就指向我们要打开的文件的FileNode
                        fileNodeOffset =
                            j * 256 + BLOCK_SIZE * root->dataPoint[i];
                        break;
                    }
        ...
        if (flag == 0) {
            ...
            if (path == NULL && fileExist == 1) {
                // 表明
                // 上面遍历的循环是由于fileExist退出的，并且此时的path是最后一个字符串了，即找到了我们要打开的文件
                // 此处是打开文件 正常退出的出口
                // 此时待打开的文件的FileNode就是nodePointer

                if (currentUid != nodePointer->belong &&
                    nodePointer->belong != -1) {
                    free(file);
                    free(block);
                    free(blockNode);
                    return -10;
                ...
                if (i == 10) {
                    // 前面的10个一级索引是用完了的
                    // 当然 有可能是前面的10个一级索引恰好存完了所有的数据，此时
                    // 没有使用二级索引 更多的情况是 使用了二级索引
                    fseek(fp, nodePointer->blockPoint * BLOCK_SIZE, SEEK_SET);
                    fread(blockNode, BLOCK_SIZE, 1, fp); // 读出二级索引表
                    int j = 0;
                    for (j = 0; j < (fileSize - BLOCK_SIZE * 10) / BLOCK_SIZE;
                         j++) {
                        // 使用的完整的二级索引
                        fseek(fp, ((long *)blockNode)[j] * BLOCK_SIZE,
                              SEEK_SET);
                        fread(contentBuf, BLOCK_SIZE, 1, fp);
                        ui->shell->insertPlainText(contentBuf);
                    }
                    ...
                    // 此时是明确的，前面的10个一级索引是没有用完的
                    // 我们从下一个一级索引中读取余下的部分 写入到shell中
                    fseek(fp, nodePointer->dataPoint[i] * BLOCK_SIZE, SEEK_SET);
                    memset(contentBuf, 0, BLOCK_SIZE);
                    fread(contentBuf, fileSize % BLOCK_SIZE, 1, fp);
                    ui->shell->insertPlainText(contentBuf);
...
int writeClose(const char *fileContent, long fileNodeOffset, FILE *fp) {
    ...
    if (fileBlock == originBlock) {
        ...
        if (i == 10) {
            ...
            // 剩下一个不完全的二级索引
            fseek(fp, ((long *)blockNode)[j] * BLOCK_SIZE, SEEK_SET);
            fwrite(fileContent + (10 + j) * BLOCK_SIZE, fileLength % BLOCK_SIZE,
                   1, fp);
        } else {
            // 10个一级索引没有用完 写入最后一个一级索引的数据
            fseek(fp, file.dataPoint[i] * BLOCK_SIZE, SEEK_SET);
            fwrite(fileContent + i * BLOCK_SIZE, fileLength % BLOCK_SIZE, 1,
                   fp);
        }
        ...
        if (i == 10) {
            ...
            for (j = 0; j < fileBlock - originBlock - 1; j++) {
                ((long *)blockNode)[j + originBlock - 10] = getBlock(
                    fp); // warning: 这个blockNode的块最后要写入到文件系统中
                fseek(fp,
                      ((long *)blockNode)[j + originBlock - 10] * BLOCK_SIZE,
                      SEEK_SET);
                fwrite(fileContent + (originBlock + j) * BLOCK_SIZE, BLOCK_SIZE,
                       1, fp);
            }
            // 最后一个块 可能是填不满的
            ((long *)blockNode)[j + originBlock - 10] = getBlock(fp);
            fseek(fp, ((long *)blockNode)[j + originBlock - 10] * BLOCK_SIZE,
                  SEEK_SET);
            ...
            fseek(fp, ((long *)blockNode)[j + originBlock - 10] * BLOCK_SIZE,
                  SEEK_SET);
            fwrite(fileContent + (fileBlock - 1) * BLOCK_SIZE,
                   fileLength - (fileBlock - 1) * BLOCK_SIZE, 1, fp);
            ...
            if (fileBlock <= 10) {
                ...
                fwrite(temp, BLOCK_SIZE, 1, fp);
                fseek(fp, file.dataPoint[j] * BLOCK_SIZE, SEEK_SET);
                fwrite(fileContent + j * BLOCK_SIZE,
                       fileLength - j * BLOCK_SIZE, 1, fp);
            } else {
                ...
                file.blockPoint = getBlock(
                    fp); // warning: 这个块 我们最后是要把数据写入的文件系统的
                memset(blockNode, 0, BLOCK_SIZE);
                for (j = 0; j < fileBlock - 1 - 10; j++) {
                    ((long *)blockNode)[j] = getBlock(fp);
                    fseek(fp, ((long *)blockNode)[j] * BLOCK_SIZE, SEEK_SET);
                    fwrite(fileContent + (j + 10) * BLOCK_SIZE, BLOCK_SIZE, 1,
                           fp);
                }
                ((long *)blockNode)[j] = getBlock(fp);
                fseek(fp, ((long *)blockNode)[j] * BLOCK_SIZE, SEEK_SET);
                ...
                fwrite(temp, BLOCK_SIZE, 1, fp);
                fseek(fp, ((long *)blockNode)[j] * BLOCK_SIZE, SEEK_SET);
                fwrite(fileContent + (j + 10) * BLOCK_SIZE,
                       fileLength - (j + 10) * BLOCK_SIZE, 1, fp);
...
int listFile(const char *dirPath, FILE *fp, Ui::MainWindow *ui) {
    ...
    while (path != NULL) {
        ...
        // 我们在创建文件/目录的时候没有使用二级索引，所以一个目录下面最多只能有10
        // × 16个 文件/目录
    ...
    QString showFile;
    QString user;
    struct SuperBlock superUser;
    fseek(fp, 0, SEEK_SET);
    fread(&superUser, sizeof(struct SuperBlock), 1, fp);
    for (int i = 0; i < 10; i++) {
        if (root->dataPoint[i] == 0) {
            break;
        }
        ...
        for (int j = 0; j < 16; j++) {
            if (nodePointer[j].valid == 1) {
                if (nodePointer[j].type == 0) {
                    // 找到一个文件
                    if (nodePointer[j].belong == -1) {
                        user = QString("Shared");
                    } else {
                        user = QString(
                            superUser.username[(int)nodePointer[j].belong]);
                    }
                    showFile = QString(nodePointer[j].filename) +
                               "\tRegular File\t" + user + "\t" +
                               QString::number(nodePointer[j].size) + "Bytes\n";
                    ui->shell->insertPlainText(showFile);
                } else {
                    showFile =
                        QString(nodePointer[j].filename) + "\tDirectory\n";
                    ui->shell->insertPlainText(showFile);
                    // 找到一个目录
                }
            }
...
int rmFile(const char *filePath, FILE *fp) {
    // 删除文件首先需要找到文件 我们在openFile的基础上进行修改
    // 只需要将原来的打开的操作改为删除的操作即可
    if (filePath[0] != '/') {
        return -1;
    }
    if (filePath[0] == '/' && filePath[1] == '/') {
...
int releaseBlock(unsigned long block, FILE *fp) {
    ...
    bitmap[bitmapLoc / 8] =
        ~(~(bitmap[bitmapLoc / 8]) | (1 << (bitmapLoc % 8)));
    fseek(fp, BLOCK_SIZE * bitmapBlock, SEEK_SET);
    fwrite(bitmap, BLOCK_SIZE, 1, fp);
    fseek(fp, offset, SEEK_SET);
    return 0;
...
int signin(const char *username, const char *password, FILE *fp) {
    int success = 0;
    long offset = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    struct SuperBlock super;
    ...
    for(int i = 0; i < MAX_USER_NUM; i++) {
        if(strcmp(super.username[i], username) == 0) {
            fseek(fp, offset, SEEK_SET);
            return -2;
        }
    }
    ...
    for (int i = 0; i < MAX_USER_NUM; i++) {
        if (super.username[i][0] == 0) {
            // 该位置还没有保存用户信息
            strcpy(super.username[i], username);
            // 下面对password做md5操作，并保存到super中
    ...
    if (success == 0) {
        ...
        fseek(fp, 0, SEEK_SET);
        fwrite(&super, sizeof(struct SuperBlock), 1, fp);
        fseek(fp, offset, SEEK_SET);
        return 0;
    }
}

int login(const char *username, const char *password, FILE *fp) {
    int success = 0;
    int offset = ftell(fp);
    ...
    fread(&super, sizeof(struct SuperBlock), 1, fp);
    for (int i = 0; i < MAX_USER_NUM; i++) {
        if (strcmp(username, super.username[i]) == 0) {
            MD5((const unsigned char *)password, strlen(password), md5);
            if (memcmp((const char *)md5, (const char *)super.password[i],
                       MD5_DIGEST_LENGTH) == 0) {
                currentUid = i;
                success = 1;
                break;
...
int shareFile(const char *filePath, FILE *fp) {
    // 使用openFile的代码，将打开操作修改为调整uid即可
    if (filePath[0] != '/') {
        return -1;
    }
    if (filePath[0] == '/' && filePath[1] == '/') {
    ...
    while (path != NULL) {
        ...
        if (flag == 0) {
            ...
            if (path == NULL && fileExist == 1) {
                ...
                nodePointer->belong = -1; // mean shared
                fseek(fp, fileNodeOffset, SEEK_SET);
                fwrite(nodePointer, sizeof(struct FileNode), 1, fp);
                return 0;
            }
