...
void TCPRdtSender::receive(const Packet &ackPkt) {
    if (countPackage != 0) {
        ...
        if (checkSum == ackPkt.checksum) {
            ...
            if (ackPkt.seqnum == ackSeq) {
                // 收到重复ACK
                countACK++;
            } else {
                countACK = 0;
                ackSeq = ackPkt.seqnum;
            }
            ...
            if (countACK == 3) {
                ...
                pns->startTimer(SENDER, Configuration::TIME_OUT, 0);
                pUtils->printPacket(
                    "收到3次重复ACK，发送方重发窗口中的第一个包",
                    sendPackage[windowBase]);
                pns->sendToNetworkLayer(RECEIVER, sendPackage[windowBase]);
            }
...
void TCPRdtSender::timeoutHandler(int seqNum) {
    pUtils->printPacket("发送方定时器时间到，重发窗口中的第一个包",
                        sendPackage[windowBase]);
    cout << RED << "当前窗口开始位置为 " << windowBase
         << "， 当前窗口中包的数量为 " << countPackage << endl
         << RESET;
    ...
    //                             sendPackage[(i + windowBase) % (SEQ_MAX +
    //                             1)]);
    //     // 发送窗口中的包
    // }
    // 注释部分为GBN中的实现
    // 下面为简化TCP实现 超时时仅重传窗口中的第一个报文
    cout << RED;
    pUtils->printPacket("发送方重发窗口中的第一个包", sendPackage[windowBase]);
    cout << RESET;
    pns->sendToNetworkLayer(RECEIVER, sendPackage[windowBase]);
}